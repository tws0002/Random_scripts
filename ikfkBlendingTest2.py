...
class IkFkBlending():
	def __init__(self):
		...
		if joint1.find("_JNT") == -1:
			mc.rename(joint1, joint1+"_JNT")
			mc.rename(joint2, joint2+"_JNT")
			mc.rename(joint3, joint3+"_JNT")
		...
		ikJoint1 = mc.duplicate(joint1, n = self.jointNames[0]+"_IK", renameChildren = True)[0]
		ikJoint2 = mc.rename(mc.listRelatives(ikJoint1, children = True)[0], self.jointNames[1]+"_IK")
		ikJoint3 = mc.rename(mc.listRelatives(ikJoint2, children = True)[0], self.jointNames[2]+"_IK")

		for joint in [ikJoint1, ikJoint2, ikJoint3]:
			self.ikJoints.append(joint)

		fkJoint1 = mc.duplicate(joint1, n = self.jointNames[0]+"_FK", renameChildren = True)[0]
		fkJoint2 = mc.rename(mc.listRelatives(fkJoint1, children = True)[0], self.jointNames[1]+"_FK")
		fkJoint3 = mc.rename(mc.listRelatives(fkJoint2, children = True)[0], self.jointNames[2]+"_FK")
	...
	def createFKControl(self):
		for joint in self.fkJoints:
			...
			mc.orientConstraint(FKControlObject,)
	...
	def generateFKControl(self):
		...
		FKControlObjectShape = mc.listRelatives(circle1, children = True)
		...
		FKControlObjectShape = mc.listRelatives(circle2, children = True)
		mc.setAttr(FKControlObjectShape[0]+".overrideEnabled", 1)
		mc.setAttr(FKControlObjectShape[0]+".overrideColor", 13)
		
		circle3 = mc.listRelatives(mc.circle( nr=(0, 0, 1), c=(0, 0, 0), r = 1)[0], children = True)[0]
		FKControlObjectShape = mc.listRelatives(circle3, children = True)
		mc.setAttr(FKControlObjectShape[0]+".overrideEnabled", 1)
		mc.setAttr(FKControlObjectShape[0]+".overrideColor", 13)
